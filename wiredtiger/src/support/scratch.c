...
__wt_buf_grow_worker(WT_SESSION_IMPL *session, WT_ITEM *buf, size_t size)
{
	 ...
	 */
	if (size > buf->memsize) {/*进行内存重分配*/
	    //__wt_realloc_aligned中buf->memsize会从新赋值为size，也就是分配的总空间大小
		if (F_ISSET(buf, WT_ITEM_ALIGNED))
			WT_RET(__wt_realloc_aligned(
			    session, &buf->memsize, size, &buf->mem)); //mem现在指向为新的空间了
		else
	...
	if (buf->data == NULL) {
		...
		if (copy_data) /*进行数据拷贝*/
			memcpy(buf->mem, buf->data, buf->size); //其实这里有重复拷贝的过程，因为前面的__wt_realloc_aligned已经拷贝过了
		buf->data = (uint8_t *)buf->mem + offset; //下次只能从buf->mem + offset处使用内存空间
	}
    ...
    )
{
	...
	*scratchp = NULL;
	...
	for (i = 0, best = slot = NULL,
		...
		if ((buf = *p) == NULL) { //这是个空闲的，没有指向任何地址，记录到slot中
			if (slot == NULL)
				slot = p;
			continue;
		}

		if (F_ISSET(buf, WT_ITEM_INUSE)) //该item在使用中
			continue;
	...
	if (best == NULL && slot == NULL) {
		...
		WT_ERR(__wt_realloc(session, &allocated,
		    ...
		    &session->scratch)); //多分配10个WT_ITEM*指针空间
#ifdef HAVE_DIAGNOSTIC
	...
	if (best == NULL) {
		...
		best = slot; //先找到这个可用的WT_ITEM*指针地址，然后下面分配一个WT_ITEM结构给它
	...
	session->scratch_cached -= (*best)->memsize; //重用的空间还剩下多少
	
	WT_ERR(__wt_buf_init(session, *best, size));//分配__wt_item的data数据空间
	F_SET(*best, WT_ITEM_INUSE); /*设置为占用状态*/
