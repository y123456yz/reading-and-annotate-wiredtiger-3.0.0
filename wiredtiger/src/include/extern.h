...
extern /*在内存中时，一般是这样使用，先获取到原来的key，加入一个新key的时候，重新分配原来+新的key总长度 __wt_ref_key(page, *page_refp, &p, &size);;
...
extern int __wt_conn_dhandle_open( //分table和file两种情况 WT_SESSION_IMPL *session, const char *cfg[], uint32_t flags) WT_GCC_FUNC_DECL_ATTRIBUTE((warn_unused_result));
extern int //例如针对某个tree或者所有tree执行checkpoint __wt_conn_btree_apply(WT_SESSION_IMPL *session, const char *uri, int (*file_func)(WT_SESSION_IMPL *, const char *[]), int (*name_func)(WT_SESSION_IMPL *, const char *, bool *), const char *cfg[]) WT_GCC_FUNC_DECL_ATTRIBUTE((warn_unused_result));
...
extern int __wt_curfile_open(WT_SESSION_IMPL *session, const char *uri, WT_CURSOR *owner, const char *cfg[], WT_CURSOR **cursorp) //file对应的cursor接口见__curfile_create WT_GCC_FUNC_DECL_ATTRIBUTE((warn_unused_result));
...
extern int //__wt_metadata_search和__wt_metadata_insert配合，一个写，一个查询 __wt_metadata_insert( WT_SESSION_IMPL *session, const char *key, const char *value) WT_GCC_FUNC_DECL_ATTRIBUTE((warn_unused_result));
...
extern int //__wt_metadata_search和__wt_metadata_insert配合，一个写，一个查询 __wt_metadata_search(WT_SESSION_IMPL *session, const char *key, char **valuep) WT_GCC_FUNC_DECL_ATTRIBUTE((warn_unused_result));
...
extern int __wt_eventv2(WT_SESSION_IMPL *session, char*func, int line, bool msg_event, int error, const char *file_name, int line_number, const char *fmt, va_list ap) WT_GCC_FUNC_DECL_ATTRIBUTE((cold)) WT_GCC_FUNC_DECL_ATTRIBUTE((warn_unused_result));
...
extern void __wt_verbose_worker(WT_SESSION_IMPL *session, const char*func, int line, const char *fmt, ...) WT_GCC_FUNC_DECL_ATTRIBUTE((format (printf, 2, 3))) WT_GCC_FUNC_DECL_ATTRIBUTE((cold));
...
extern void //__wt_thread_group_stop_one和__wt_thread_group_start_one对应，分别对应激活和停止 __wt_thread_group_start_one( WT_SESSION_IMPL *session, WT_THREAD_GROUP *group, bool is_locked);
...
extern void //__wt_txn_set_commit_timestamp和__wt_txn_clear_commit_timestamp对应 __wt_txn_set_commit_timestamp(WT_SESSION_IMPL *session);
extern void __wt_txn_clear_commit_timestamp(WT_SESSION_IMPL *session);
extern void //__wt_txn_begin->__wt_txn_config->__wt_txn_set_read_timestamp __wt_txn_set_read_timestamp(WT_SESSION_IMPL *session);
