...
__wt_txn_modify(WT_SESSION_IMPL *session, WT_UPDATE *upd)
{
	...
	if (WT_IS_METADATA(session->dhandle)) {
		if (!F_ISSET(session, WT_SESSION_LOGGING_INMEM))
	     ...
	     //在begin_transaction后通过session->timestamp_transaction设置时间撮，紧接着进行各种操作，这些操作会执行该函数，这时候会满足该条件，即:
	     /*
         ...
         update()
         。。。
         session->commit_transaction()  该模型情况下会走到这里
	     */
	     //最终影响该udp的事务可见性，见__wt_txn_upd_visible  __wt_txn_upd_visible_all
...
/*
timestamp:对应upd链表上各个Value节点的timestamp(也就是该value对应op操作所在事务commit_transaction()时候的commit_timestamp时间)
txn->read_timestamp:读操作对应session指定的read_timestamp，一般在读的时候由mongodb指定
*/
 ...
 */ /*检查事务id是否对当前session对应事务可见*/ 
//__wt_txn_read->__wt_txn_upd_visible(例如遍历cusor获取kv过程)
//__wt_row_modify->__wt_txn_update_check->__wt_txn_upd_visible (根据key查询的流程，事务生效过程)
static inline bool
__wt_txn_upd_visible(WT_SESSION_IMPL *session, WT_UPDATE *upd)
{
    //当同一时刻有多个session对该upd对应的key进行更新的时候，每个session对应的更新的timestamp是不一样的
...
__wt_txn_update_check(WT_SESSION_IMPL *session, WT_UPDATE *upd)
{
	...
	txn = &session->txn;
	if (txn->isolation == WT_ISO_SNAPSHOT)
		while (upd != NULL && !__wt_txn_upd_visible(session, upd)) { //udp对应的id事务可见，退出while
