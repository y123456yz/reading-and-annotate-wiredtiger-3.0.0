 ...
 * the buffer.
 */
/*尝试重分配ITEM中的mem，重新分配的data空间*/
static inline int
__wt_buf_grow(WT_SESSION_IMPL *session, WT_ITEM *buf, size_t size)
{
    //size > buf->memsize说明需要扩内存，!WT_DATA_IN_ITEM(buf)说明还没有给item分配data数据空间
...
__wt_scr_free(WT_SESSION_IMPL *session, WT_ITEM **bufp)
{
	...
	*bufp = NULL;
	...
	if (session->scratch_cached + buf->memsize >=
	    S2C(session)->session_scratch_max) { //scratch可用空间不能超过上限
		__wt_free(session, buf->mem);
		buf->memsize = 0;
	} else
    //没有超过上限，则计算这个可重用的空间，并没有释放空间，而是给其他人继续重用，避免反复分配空间影响效率
		session->scratch_cached += buf->memsize; 
		
	buf->data = NULL;
	buf->size = 0;
	F_CLR(buf, WT_ITEM_INUSE);//可继续使用该item了
}
