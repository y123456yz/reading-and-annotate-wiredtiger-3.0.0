...
struct __wt_btree { //btree创建空间在__wt_conn_dhandle_alloc    __wt_btree.root为树的跟节点
    //赋值见__wt_btree_open
    ...
    /*行存储时的比较器*/ //如果没指定，则一般用默认的，见__wt_compare
	WT_COLLATOR *collator;		/* Row-store comparator */
	/*如果这个值为1，表示比较器需要进行free*/
	int collator_owned;		/* The collator needs to be freed */


    /*btree索引文件ID,主要用于redo log的推演, 实际上是checkpoint id，根据该checkpoint id获取某个时间点的快照，然后结合journal log增量恢复*/
...
#define	WT_SPLIT_DEEPEN_PER_CHILD_DEF	100
	...
	///*btree->split_pct是page分裂阈值的百分比，每次分裂的时候都分裂成最大size的这个百分数大小*/
	//见__wt_split_page_size
    ...
    /*block manager句柄 从而让btree与block文件对应起来 */
    ...
    //赋值见__checkpoint_update_generation
