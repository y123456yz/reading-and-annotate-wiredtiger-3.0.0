...
__session_add_dhandle(WT_SESSION_IMPL *session)
{
	...
	dhandle_cache->dhandle = session->dhandle; //直接把session->dhandle当前的这个handle赋值给这个新的dhandle_cache
 ...
 */
 //从session->dhandles和dhhash中清除dhandle_cache
static void
__session_discard_dhandle(
 ...
 *	Search for a data handle in the session cache.
 根据dhandle name和checkpoint name找到对应的dhandle
 通过dhandle name和checkpoint name查找对应的dhandle
...
retry:	TAILQ_FOREACH(dhandle_cache, &session->dhhash[bucket], hashq) {
		...
		if (strcmp(uri, dhandle->name) != 0) //匹配uri和dhandle对应的name
			continue;
		if (checkpoint == NULL && dhandle->checkpoint == NULL)
			break;
		if (checkpoint != NULL && dhandle->checkpoint != NULL &&
		    strcmp(checkpoint, dhandle->checkpoint) == 0) //匹配checkpoint
			break;
	}
...
__session_dhandle_sweep(WT_SESSION_IMPL *session)
{
	 ...
	 * do it again.
	 */
	 //定期清理handle,如果最近清理过，则不需要在清理，保证一定时间内清理一次
