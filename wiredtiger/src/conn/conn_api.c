...
/* 从home目录读取wiredtiger的配置文件filename信息，读取到cbuf中，同事追加到cfg */
static int
...
__conn_config_env(WT_SESSION_IMPL *session, const char *cfg[], WT_ITEM *cbuf)
{
    ...
    //获取WIREDTIGER_CONFIG环境变量信息存入cbuf
	...
	WT_ERR(__wt_buf_set(session, cbuf, env_config, len + 1));//拷贝env_config到cbuf item中
 ...
 */
/* 设置使用wiredtiger引擎的工作路径，设置WT_CONNECTION_IMPL.home成员为home */
static int
    ...
    WT_CONNECTION_IMPL *conn, WT_EVENT_HANDLER *event_handler)
{
	...
	__wt_os_stdio(session); //session输出打印相关的
    ...
    const char *config, WT_CONNECTION **wt_connp)
{
	...
	*wt_connp = NULL;
	...
	cfg[1] = config;
	WT_ERR(__wt_scr_alloc(session, 0, &i1)); //获取item空间，data部分未0
	WT_ERR(__conn_config_env(session, cfg, i1)); //获取WIREDTIGER_CONFIG信息追加到cfg中
	...
	WT_ERR(__wt_config_gets(session, cfg, "error_prefix", &cval));
	if (cval.len != 0) {
		__wt_free(session, conn->error_prefix); //是否前面第一个error_prefix，用这个新的，这个配置级别应该更高
		WT_ERR(__wt_strndup(
		    session, cval.str, cval.len, &conn->error_prefix));
	}
