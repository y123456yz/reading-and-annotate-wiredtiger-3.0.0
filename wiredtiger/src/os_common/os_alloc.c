 ...
 */
//分配number个长度为size的连续空间，然后通过retp返回空间地址
...
//按照session->buffer_alignment对齐重分配bytes_to_allocate字节空间，如果原来retp有bytes_allocated_ret字节，则需要拷贝到重分配的空间头部
//总结:以前retp有*bytes_allocated_ret空间，现在realloc到bytes_to_allocate空间，同时保证session->buffer_alignment字节对其
int
    ...
    size_t *bytes_allocated_ret, size_t bytes_to_allocate, void *retp)
{
	...
	if (session != NULL && S2C(session)->buffer_alignment > 0) {
		...
		if (p != NULL)
			memcpy(newp, p, bytes_allocated); //把原有retp的bytes_allocated_ret字节拷贝到新空间中
		__wt_free(session, p); //把以前的retp空间释放
		p = newp;

		/* Update caller's bytes allocated value. */
		if (bytes_allocated_ret != NULL)
			*bytes_allocated_ret = bytes_to_allocate; //新空间为bytes_to_allocate字节
