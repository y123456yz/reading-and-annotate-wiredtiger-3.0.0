...
__evict_exclusive(WT_SESSION_IMPL *session, WT_REF *ref)
{
    ...
    //if (__wt_hazard_check_assert(session, ref, NULL) == true) //yang add change todo 
        return (0);
...
int
__wt_page_release_evict(WT_SESSION_IMPL *session, WT_REF *ref, uint32_t flags, bool urgent_evict)
{
    WT_BTREE *btree;
    WT_DECL_RET;
    ...
    //yang add change
    if (false && urgent_evict && previous_state == WT_REF_MEM && !F_ISSET(cache, WT_CACHE_EVICT_ALL)) {
        __wt_verbose_info(
            session, WT_VERB_TRANSACTION, "yang test ..1....__wt_page_release_evict.... %p %p", ref, ref->page);
        evict_flags = LF_ISSET(WT_READ_NO_SPLIT) ? WT_EVICT_CALL_NO_SPLIT : 0;
        FLD_SET(evict_flags, WT_EVICT_CALL_URGENT);
        FLD_SET(ref->evict_flags, evict_flags);

        printf("yang test ........evict_flags.........:%u  %u\r\n", ref->evict_flags, WT_EVICT_CALL_URGENT);
        ...
        __wt_verbose_info(
            session, WT_VERB_TRANSACTION, "yang test ..2....__wt_page_release_evict.... %p %p", ref, ref->page);
    }
    ...
    WT_UNUSED(urgent_evict);
    //WT_IGNORE_RET(__wt_msg(session, "yang test ...............__wt_page_release_evict........................2..ref->state:%d", ref->state));
...
__wt_evict(WT_SESSION_IMPL *session, WT_REF *ref, uint8_t previous_state, uint32_t flags)
{
    ...
    __wt_verbose_info(
        session, WT_VERB_EVICT, "__wt_evict evict page %p (%s) memory_footprint:%d, page->dsk->mem_size:%d, entries:%d, falgs:%u", (void *)page, 
        __wt_page_type_string(page->type), (int)page->memory_footprint, page->dsk ? (int)page->dsk->mem_size : 0, (int)page->entries, flags);
   ...
   // printf("yang test .............__wt_evict......:%s\r\n", __wt_page_type_string(page->type));
     ...
     */
    //pagesplitmempagesplit
    //  1. pagesplitmempage(80% * maxmempage)splite__wt_leaf_page_can_split
    //  2. maxleafpage * 25KV 
    //  3. 30KVpagemaxleafpage
    if (inmem_split) {//pagesplit
        WT_ERR(__wt_split_insert(session, ref));
        goto done;
    }

    //pagesplitmempage(80% * maxmempage)pagespliteWT_PAGE_SPLIT_INSERTreconcile evict

    //
    /* No need to reconcile the page if it is from a dead tree or it is clean. */
    if (!tree_dead && __wt_page_is_modified(page)) {
        //__evict_reconcile:
        //  evict reconcilemaxmempagepagemaxleafpage_precomp(__wt_rec_split_init)multi_next
        //  multi[]__evict_page_dirty_updatepagepageindex
    ...
    if (__wt_ref_is_root(ref))
        ...
        //__evict_reconcilepagepagepage(mod->mod_multi), split
...
//checkpoint:
//    __checkpoint_tree->__wt_sync_filemulti_nextmulti[multi_next]__evict_page_dirty_updatepage
//    __wt_rec_row_int->__rec_row_mergemulti[multi_next]
//evict reconcile:
//    __evict_reconcile: pagesplit_sizechunk, multi_nextmulti[multi_next]
//    __evict_page_dirty_update: __evict_reconcilemulti[multi_next]multi_nextpagepage

//__evict_reconcile: pagesplit_sizechunk, multi_nextmulti[multi_next]
//__evict_page_dirty_update: __evict_reconcilemulti[multi_next]multi_nextpagepage

//__wt_evict->__evict_page_dirty_update
//__evict_page_dirty_update__evict_reconcile
static int
__evict_page_dirty_update(WT_SESSION_IMPL *session, WT_REF *ref, uint32_t evict_flags)
{
  ...
  //  printf("yang test ............__evict_page_dirty_update..........\r\n");
    ...
    switch (mod->rec_result) {//__rec_write_wrapup
    case WT_PM_REC_EMPTY:
        ...
        WT_RET_BUSY_OK(ret);
        break;
    case WT_PM_REC_MULTIBLOCK: //__evict_reconcilemulti splitmulti_nextmulti
        /*
...
//__wt_evict: inmem_splitpage__wt_split_insert(split-insert)
//__evict_page_dirty_update(__evict_reconcile): pagepage,__wt_split_multi(split-multi)
//__evict_page_dirty_update(__evict_reconcile): __wt_split_reverse(reverse-split)
//__evict_page_dirty_update(__evict_reconcile):__wt_split_rewrite(split-rewrite)

//__evict_reconcile: pagesplit_sizechunk, multi_nextmulti[multi_next]
//__evict_page_dirty_update: __evict_reconcilemulti[multi_next]multi_nextpagepage

//__evict_force_checkpagesplit evict(__wt_evict)reconcile evict(__evict_reconcile)
static int
