...
//__evict_reconcileÖÐÈç¹ûÊÇleaf pageÉèÖÃ¸Ã±êÊ¶, __wt_sync_fileÒ²»áÉèÖÃ
#define WT_REC_HS 0x040u
#define WT_REC_IN_MEMORY 0x080u
//Ò»°ãreconcile¶¼»áÓµÓÐ¸Ã±êÊ¶
...
//WT_PAGE_HEADER(__wt_page_header)ÔÚ__rec_split_write_headerÖÐÍê³É¸³Öµ£¬¶ÔÓ¦ÄÚ´æÎ»ÖÃ¼ûWT_BLOCK_HEADER_REF
//WT_BLOCK_HEADER(__wt_block_header)¸³ÖµÔÚÐ´´ÅÅÌÍê³Éºó£¬ÔÚ__block_write_off¸³Öµ,¶ÔÓ¦ÄÚ´æÆ«ÒÆ¼ûWT_PAGE_HEADER_BYTE_SIZE
//Èç¹ûÅäÖÃÁËÑ¹Ëõ£¬Ôò__wt_page_header¼ÇÂ¼µÄÊÇÑ¹ËõÇ°µÄÊý¾ÝÐÅÏ¢,__wt_block_header¼ÇÂ¼µÄÊÇÑ¹ËõºóµÄÊý¾ÝÐÅÏ¢

 
//Ò»¸öpageÊý¾ÝÔÚ´ÅÅÌÖÐÁ¬Ðø¿Õ¼äÄÚÈÝ: __wt_page_header + WT_BLOCK_HEADER + WT_CELL
...
struct __wt_page_header {
    ...
    uint64_t recno; /* 00-07: column-store starting recno */ //ÐÐ´æ´¢¸ÃÄÚÈÝÎªWT_RECNO_OOB
    ...
    union {
        //__wt_rec_split_finishµ±Ç°reconcile´¦ÀíµÄpageÉÏÃæµÄKºÍV×ÜÊý
        uint32_t entries; /* 20-23: number of cells on page */
        uint32_t datalen; /* 20-23: overflow data length */
    } u;

    //Ò²¾ÍÊÇ¶ÔÓ¦pageÀàÐÍ£¬ÀýÈçWT_PAGE_ROW_LEAF WT_PAGE_BLOCK_MANAGER(±íÊ¾checkpointµÄext)
...
//dsk¿ªÊ¼Ìø¹ýpage header + block header,Ò²¾ÍÊÇÕâ¸öpage¶ÔÓ¦µÄÊµ¼ÊÊý¾ÝÆðÊ¼µØÖ·
 ...
 */ //Ò»¸öpage²ð·ÖÎª¶à¸öpageÊ±ºòÓÃµ½£¬¿ÉÒÔ²Î¿¼__rec_split_dump_keysµÄ±éÀú,__rec_split_writeÕâÀï´´½¨¿Õ¼äºÍ¸³Öµ
//¹ÜÀí´ÅÅÌpage imageµÄÄÚ´æÔªÊý¾ÝÐÅÏ¢£¬Ò»¸ö__wt_multi¶ÔÓ¦Ò»¸öpag ´ÅÅÌimage
struct __wt_multi {
    /*
     * Block's key: either a column-store record number or a row-store variable length byte string.
     */
    //page²ð·Öºó¶ÔÓ¦µÄref key£¬²Î¿¼__rec_split_dump_keysµÄ´òÓ¡£¬¸³ÖµµÄµØ·½ÔÚ__rec_split_write
    union {
        uint64_t recno;
        WT_IKEY *ikey;
    } key;
     ...
     * memory.
     */
    //Í¨¹ýreconcile°ÑÒ»¸öchunkÊý¾ÝÐ´Èë´ÅÅÌºó£¬»á¿½±´Ò»·Ýµ½disk_image, ²Î¿¼__rec_split_write
    //Ò²¾ÍÊÇdisk_imageÄÚ´æÒ»¸öÍêÕûµÄchunk(WT_PAGE_HEADER_SIZE + WT_BLOCK_HEADER_SIZE + Êµ¼ÊÊý¾Ý)
    //×îÖÕÔÚ__wt_page_inmemÖÐ¸³Öµ¸øpage->dsk, È»ºóÔÚ__wt_multi_to_ref->__split_multi_inmem->__wt_page_inmem->__inmem_row_leaf½âÎö
    //´ÅÅÌpage->dskÖÐµÄKºÍVµØÖ·ÐÅÏ¢´æÈëµ½page->pg_row, ×îºóÔÚ__wt_multi_to_refÖÐÊÍ·Ådisk_image

    //´ÓÉÏÃæµÄ±¸×¢¿ÉÒÔ¿´³ö£¬reconcileÒ»¸ömulti¶ÔÓ¦Êý¾ÝÐ´Èë´ÅÅÌºó£¬»áÔÚ¿½±´Ò»·Ýµ½disk_imageÖÐ£¬×îÖÕÔÚ__wt_page_inmemÖÐ¸³Öµ¸øpage->dsk
    //È»ºóÔÚ__inmem_row_leaf½âÎö³ö´ÅÅÌÉÏµÄKºÍVµØÖ·±£´æµ½page->pg_row[]ÖÐ, ×îºóÊÍ·Ådisk_image¿Õ¼ä£¬Ò²¾ÍÊÇdisk_imageÖ»ÊÇÒ»¸öÁÙÊ±
    //±äÁ¿±£´æÐ´Èë´ÅÅÌµÄËùÓÐ·â°üÊý¾Ý£¬×îÖÕÄ¿µÄÊÇÎªÁË»ñÈ¡page´ÅÅÌÉÏK»òÕßVÊý¾Ý±£´æµ½page->pg_row[]Êý×éÖÐ
    void *disk_image;
...
//ÀýÈçleaf pageÖÐÌøÔ¾±íÖÐ½ÚµãÄÚÈÝ¶¼ÔÚ¸ÃmodifyÖÐ, modÏà¹Ø¿Õ¼äÊÍ·Å¼û__free_page_modify
struct __wt_page_modify {
    /* The first unwritten transaction ID (approximate). */
    uint64_t first_dirty_txn;

    /* The transaction state last time eviction was attempted. */
    //__reconcile_save_evict_stateÖÐ¸³Öµ
...
#define mod_disk_image u1.r.disk_image
        ...
        //¹ÜÀí´ÅÅÌpage imageµÄÄÚ´æÔªÊý¾ÝÐÅÏ¢£¬Ò»¸ö__wt_multi¶ÔÓ¦Ò»¸öpag ´ÅÅÌimage
        struct {//×¢Òâ__wt_reconcile.multiºÍ__wt_page_modify.mod_multi_entries.multiµÄÇø±ðÁªÏµ
            //__rec_write_wrapupÖÐ¸³Öµ
...
//¸³Öµ²Î¿¼__rec_write_wrapup£¬Ò»´Îreconcile½áÊøºó£¬reconcileµÄmultiÐÅÏ¢×ª´æµ½mod->mod_multiÖÐ, Ò²¾ÍÊÇr->multi_nextÊý
 ...
 */
//±éÀú»ñÈ¡internal pageËù°üº¬µÄËùÓÐ×Óref
...
//Ö¸Ïò¸ÃpageµÄref£¬²Î¿¼Í¼ÐÎ»¯https://github.com/wiredtiger/wiredtiger/wiki/In-Memory-Tree-Layout
#define pg_intl_parent_ref u.intl.parent_ref
#undef pg_intl_split_gen
//¼ÇÂ¼¸Ãinternal page·ÖÁÑµÄ´ÎÊý  __split_parentÖÐ¸³Öµ
#define pg_intl_split_gen u.intl.split_gen

/*
 * WT_PAGE --
 *	The WT_PAGE structure describes the in-memory page information.
 */ //__wt_page_alloc·ÖÆ¬¿Õ¼äºÍ¸³Öµ __wt_page_out¶Ôpage½øÐÐ¿Õ¼äÊÍ·Å
struct __wt_page {
    ...
    union {
         ...
         */
        //ÅäºÏÍ¼ÐÎ»¯ÔÄ¶Áhttps://github.com/wiredtiger/wiredtiger/wiki/In-Memory-Tree-Layout
        struct {
            //Ö¸Ïò¸¸½ÚµãµÄpage
 ...
 * destroyed.
 */
//²Î¿¼BTREEÊýÍ¼ÐÎ»¯ https://github.com/wiredtiger/wiredtiger/wiki/In-Memory-Tree-Layout
//ÎªºÎÊÇsafe°²È«µÄ£¬¿ÉÒÔ²Î¿¼WT_ENTER_PAGE_INDEX  WT_LEAVE_PAGE_INDEX  WT_WITH_PAGE_INDEX
//Ö®Àà¿ÉÒÔÑéÖ¤Ò»ÏÂ£¬¼ÙÉèÄ³¸öÓÃ»§Ïß³Ì²Ù×÷µÄpage³¬¹ýÏÞÖÆ£¬ÔÚevictÖÐ»áspliteÖØÐÂÉú³ÉÐÞ¸Äindex£¬ÕâÊ±ºòÓÃ»§Ïß³ÌµÄevictÂß¼­ÖÐÎÒÃÇsleep£¬
...
//pageµÄ__indexÊý×é¸³Öµ£¬¼û__wt_page_alloc
...
//Ö¸Ïò¸Ãpage´æ´¢µÄÕæÊµÊý¾Ý£¬pg_row[]Êý×é(Êý×é´óÐ¡page->entries)¿Õ¼ä·ÖÅä¼û__wt_page_alloc,±£³ÖK»òÕßVµÄ´ÅÅÌµØÖ·£¬Ã¿¸öKV¸³Öµ²Î¿¼__inmem_row_leaf
//´ÅÅÌÉÏµÄÊý¾Ý×îÖÕÓÐÒ»·ÝÍêÈ«Ò»ÑùµÄÄÚ´æÊý¾Ý´æÔÚpage->dskµØÖ·¿ªÊ¼µÄÄÚ´æ¿Õ¼ä£¬page->pg_row[]Êý×éÃ¿¸öK»òÕßV³ÉÔ±Êµ¼ÊÉÏÖ¸ÏòÀëpage->dskÆðÊ¼Î»ÖÃµÄ¾àÀë£¬²Î¿¼__wt_cell_unpack_safe
//page->dsk´æ´¢ext´ÅÅÌÍ·²¿µØÖ·£¬page->pg_row[]´æ´¢Êµ¼ÊµÄK»òÕßVÏà¶ÔÍ·²¿µÄ¾àÀë£¬Í¨¹ýpage->dskºÍÕâ¸ö¾àÀë¾Í¿ÉÒÔÈ·¶¨ÔÚ´ÅÅÌextÖÐµÄÎ»ÖÃ,²Î¿¼__wt_page_inmem
#define pg_row u.row
...
#define pg_var_nrepeats u.col_var.repeats->nrepeats
    ...
    //´ú±íÔÚ´ÅÅÌpg_rowÉÏÃæµÄKV×ÜÊý,¿ÉÒÔ²Î¿¼__wt_evict->__evict_page_dirty_update->__wt_split_multi->__split_multi_lock
    //->__split_multi->__wt_multi_to_ref->__split_multi_inmem->__wt_page_inmem->__wt_page_alloc
    //´ÅÅÌÉÏpg_row[]ÖÐµÄKVÊýÁ¿
...
//__evict_push_candidateÖÃÎ»£¬ÌôÑ¡³ö½øÈëevict¶ÓÁÐµÄpage±êÊ¶
...
#define WT_PAGE_UPDATE_IGNORE 0x100u      /* Ignore updates on page discard */
    ...
    //__wt_cache_page_inmem_decrÖÐ×ö¼õ·¨£¬__wt_cache_page_inmem_incrÖÐ×ö¼Ó·¨
    //¸Ãpage¶ÔÓ¦µÄÄÚ´æ£¬°üÀ¨Ð´Èë´ÅÅÌµÄ£¬¿ÉÒÔ²Î¿¼__evict_force_check
    size_t memory_footprint; /* Memory attached to the page */

    /* Page's on-disk representation: NULL for pages created in memory. */
    //__split_multi_inmem->__wt_page_inmem¿ÉÒÔ¿´³ö£¬Êµ¼ÊÉÏÄÚ´æÒ²ÓÐÒ»·Ý´ÅÅÌÍêÈ«Ò»ÑùµÄÄÚ´æÊý¾Ý£¬Ò²¾ÍÊÇdisk_image£¬×îÖÕÄÚ´æÖÐµÄÊý¾Ý¼ÇÂ¼µ½dsk
    //¸ÃpageÓÐÊý¾Ý´æ´¢ÔÚ´ÅÅÌÉÏ£¬´ÅÅÌµÄ½á¹¹ÐÅÏ¢Î»ÖÃ, Ö¸ÏòÒ»¸ö´ÅÅÌchunk(WT_PAGE_HEADER_SIZE + WT_BLOCK_HEADER_SIZE + Êµ¼ÊÊý¾Ý)µÄÍ·²¿WT_PAGE_HEADER_SIZE

    //ÔÚ__split_multi_inmem->__wt_page_inmemÖÐ¸³Öµ¸øÁËpage->dsk, ²¢ÔÚÖÃÎªdisk_image=NULL£¬ ËùÒÔÖ¸ÏòµÄÄÚ´æ¿Õ¼äÊµ¼ÊÉÏ±»page->dsk¼Ì³ÐÁË
    //Ò²¾ÍÊÇ¸ÃpageÔÚ´ÅÅÌÉÏµÄÊý¾ÝÍ¬Ñù»á´æÒ»·Ýµ½ÄÚ´æÖÐ£¬Ò²¾ÍÊÇÁ½·ÝÊý¾Ý

    //Ëæ×Åsplite²ð·ÖÍê³É£¬ÀýÈç´ÓÀÏpage²ð·ÖÎª10¸öÐÂpage£¬ÀÏpage×îÖÕ»áÍ¨¹ý__wt_page_outÊÍ·Å
    //page->dsk´æ´¢ext´ÅÅÌÍ·²¿µØÖ·£¬page->pg_row[]´æ´¢Êµ¼ÊµÄK»òÕßVÏà¶ÔÍ·²¿µÄ¾àÀë£¬Í¨¹ýpage->dskºÍÕâ¸ö¾àÀë¾Í¿ÉÒÔÈ·¶¨ÔÚ´ÅÅÌextÖÐµÄÎ»ÖÃ,²Î¿¼__wt_page_inmem
    //__wt_page_out->__wt_overwrite_and_free_lenÖÐÊÍ·Å

    //__wt_cache_bytes_imageÖÐ¼ÇÂ¼×ÜµÄimageÄÚ´æÏûºÄ£¬Ò²¾ÍÊÇÐ´ÈëextÊ±ºòÔÚÄÚ´æÖÐÊµ¼ÊÉÏÓÐÍ¬ÑùÒ»·ÝµÄÄÚ´æ¼ÆËãÍ³¼Æ

     //ÅäºÏ__wt_rec_cell_build_addrÔÄ¶Á£¬[page->dsk, page->dsk + page->dsk->mem_size]¶ÔÓ¦´ÅÅÌÒ»¸öext¿é
    //Õâ¸öextÖÐ´æ´¢µÄÊÇÒ»¸öÒ»¸öµÄWT_ADDR£¬Ò»¸öWT_ADDR¶ÔÓ¦Ò»¸ö×ÓpageµÄ´ÅÅÌÔªÊý¾Ý
    const WT_PAGE_HEADER *dsk; //¸³Öµ¼û__wt_page_inmem£¬Ö¸Ïò´ÅÅÌÊý¾Ý

    /* If/when the page is modified, we need lots more information. */
    //__wt_page.modify ¸³Öµ¼û__wt_page_modify_init
    //ÀýÈçleaf pageÖÐÌøÔ¾±íÖÐ½ÚµãÄÚÈÝ¶¼ÔÚ¸ÃmodifyÖÐ£¬modÏà¹Ø¿Õ¼äÊÍ·Å¼û__free_page_modify£¬WT_PAGE_MODIFY page->modifyÏà¹Ø¿Õ¼äÊÍ·Å£¬°üÀ¨mod_row_insert mod_row_update mod_multiµÈ
 ...
 */
//³õÊ¼Öµ, ÀýÈçÒ»¸öpage½øÐÐevictµÄÊ±ºò£¬´ÓÒ»¸öpage²ð·ÖÎª¶à¸öpage,¶à³öÀ´µÄpageµÄ³õÊ¼ÖµÒ²¾ÍÊÇWT_READGEN_NOTSET
...
//Âú×ãÕâ¸öÌõ¼þÔÚ__evict_entry_priorityÖÐÖ±½ÓÆÀ·ÖÎªWT_READGEN_OLDEST
//ÆÀ·ÖÔÚÕâ¸ö·¶Î§µÄpageÐèÒªÁ¢Âíreconcile£¬
// ÆÀ·ÖÔ½µÍ£¬ËµÃ÷½üÆÚ¸ÃpageÔ½Ã»ÓÐ±»·ÃÎÊ£¬ÀýÈç2µã·ÃÎÊµÄpage1±È1µã·ÃÎÊµÄpage2µÄÆÀ·Ö»á¸ß£¬²Î¿¼__wt_cache_read_gen_bump __wt_cache_read_gen_new
...
#define WT_READGEN_STEP 100
    //__wt_cache.read_gen´ú±íÈ«¾ÖµÄread_gen£¬page.read_gen´ú±íÖ¸¶¨±íµÄ
     //__wt_cache_read_gen_new: Èç¹ûread_genÃ»ÓÐÉèÖÃ£¬Ôòevict serverÏß³ÌÔÚ__evict_walk_treeÖÐÑ¡³öÐèÒªevictµÄpageºóÍ¨¹ý¸Ãº¯ÊýÉú³Épage read_gen
    //__wt_cache_read_gen_bump: evict worker»òÕßappÓÃ»§Ïß³ÌÔÚ__wt_cache_read_gen_bumpÖÐ´Ó¶ÓÁÐÏû·ÑÕâ¸öpage reconcileµÄÊ±ºò¸³Öµ

    //__wt_page_alloc  __wt_page_evict_soonÖÐ³õÊ¼ÖµWT_READGEN_NOTSET£¬__wt_cache_read_gen_bump __wt_cache_read_gen_newÖÐÐÞ¸Ä¸³Öµ
    //´ú±í½üÆÚÊÇ·ñÓÃ»§Ïß³ÌÓÐ·ÃÎÊ¸Ãpage£¬Èç¹ûÖµÔ½´ó£¬ËµÃ÷½üÆÚ¸Ãpage±»ÓÃ»§Ïß³Ì·ÃÎÊ»îÔ¾£¬²Î¿¼__wt_cache_read_gen_new
    uint64_t read_gen;//evict serverÏß³ÌÍ¨¹ý¸ÃÖµ¶ÔÌôÑ¡µÄpage½øÐÐÆÀ·Ö£¬¼û__evict_entry_priority

    uint64_t cache_create_gen; /* Page create timestamp */
    //¸³Öµ¼û__evict_walk_tree£¬Ò²¾ÍÊÇµÚ¼¸ÂÖ__evict_passµÄÊ±ºò¸Ãpage±»ºóÌ¨evict serverÏß³ÌÑ¡ÖÐÌÔÌ­µÄ
...
struct __wt_ref {
    ...
    //´ú±í¸ÃrefÔÚ¸¸page index[]Êý×éµÄÎ»ÖÃ
...
//±êÊ¶¸ÃpageÊý¾ÝÔÚdiskÖÐ,evict°ÑÄÚ´æpageÐ´Èë´ÅÅÌºóÔÚ__wt_multi_to_refÖÐÖÃÎª¸Ã×´Ì¬
#define WT_REF_DISK 0       /* Page is on disk */
//__btree_tree_open_empty´´½¨root pageµÄÊ±ºò³õÊ¼»¯Îª¸ÃÖµ
#define WT_REF_DELETED 1    /* Page is on disk, but deleted */
#define WT_REF_LOCKED 2     /* Page locked for exclusive access */
//ÐÂ´´½¨µÄleaf page¾Í»áÉèÖÃÎª¸Ã×´Ì¬  __wt_btree_new_leaf_page´´½¨leaf pageºó£¬¸Ãleaf page¶ÔÓ¦×´Ì¬ÎªWT_REF_MEM
//µ±reconcile evict²ð·ÖpageÎª¶à¸ö£¬²¢ÇÒÐ´Èë´ÅÅÌext£¬ÕâÊ±ºòpage×´Ì¬½øÈëWT_REF_DISK, µ±unpack½â°ü»ñÈ¡µ½¸ÃextµÄËùÓÐK»òÕßVÔÚÏà±ÈextÍ·²¿
//Æ«ÒÆÁ¿ºó£¬ÖØÐÂÖÃÎªWT_REF_MEM×´Ì¬£¬±íÊ¾ÎÒÃÇÒÑ¾­»ñÈ¡µ½extÖÐ°üº¬µÄËùÓÐKºÍV´ÅÅÌÔªÊý¾ÝµØÖ·´æ´¢µ½ÁËÄÚ´æpg_rowÖÐ£¬²Î¿¼__wt_multi_to_ref
...
#define WT_REF_SPLIT 4      /* Parent page split (WT_REF dead) */
     ...
     */
    //Í¨¹ýref->addr¿ÉÒÔÅÐ¶Ï³ý¸Ãref¶ÔÓ¦pageÊÇ·ñÂÞÅÌÁË __wt_ref_block_free
    //ÀýÈçevict reconcileÁ÷³ÌÖÐµÄ__wt_multi_to_ref£¬Ö¸Ïò¸Ãpage¶ÔÓ¦µÄ´ÅÅÌextÔªÊý¾ÝÐÅÏ¢WT_ADDR(objectid offset size  checksum)
    //Èç¹ûÊÇroot page£¬Æäaddr=NULL, Èç¹ûpageÔÚÄÚ´æÖÐ£¬ÀýÈçinternal page(²»°üÀ¨root page)»òÕßleaf pageÔÚÄÚ´æÖÐ£¬»¹Ã»ÓÐ³Ö¾Ã»¯

    //ÖØÆôÊ±ºò´Ó__inmem_row_intÖÐ´ÓrootÔªÊý¾ÝextÖÐ¼ÓÔØÃ¿¸öpageµÄ´ÅÅÌaddrÐÅÏ¢
//¿ÉÒÔ²Î¿¼ÔÚ__wt_sync_fileÖÐ»á±éÀúËùÓÐµÄbtree¹ý³Ì£¬¿´__wt_sync_fileÊÇÈçºÎ±éÀúbtreeµÄ£¬È»ºó×ßµ½ÕâÀï
/*
                        root page
                        /         \
                      /             \
...
ÉÏÃæÕâÒ»¿ÃÊ÷µÄ±éÀúË³Ðò: leaf1->leaf2->internal1->leaf3->leaf4->internal2->root
*/
//´ÓÉÏÃæµÄÍ¼¿ÉÒÔ¿´³ö£¬internal page(root+internal1+internal2)×Ü¹²Èý´Î×ßµ½ÕâÀï, internal1¼ÇÂ¼leaf1ºÍleaf2µÄpage addrÔªÊý¾Ý[ref key, leaf page extÔªÊý¾Ý]
//  internal2¼ÇÂ¼leaf3ºÍleaf4µÄpage addrÔªÊý¾Ý[ref key, leaf page extÔªÊý¾Ý],
//  root¼ÇÂ¼internal1ºÍinternal2µÄ addrÔªÊý¾Ý[ref key, leaf page extÔªÊý¾Ý],
...
#undef ref_ikey
////WT_REF.ref_ikey ref¶ÔÓ¦µÄkey£¬__wt_row_ikey  __wt_ref_key_onpage_setÖÐ¸³Öµ
//¼ÇÂ¼µÄÊÇ¶ÔÓ¦pageµÄ×îÐ¡key
 ...
 */
//Ò²¾ÍÊÇ±êÊ¶rip¶ÔÓ¦KVÔÚpageÄÚ´æpg_rowÖÐµÄÓÎ±ê
 ...
 */ //__wt_row_ikey_alloc²Î¿¼£¬ref´æµÄÕæÊµpageÉÏ×îÐ¡Êý¾Ýkey
struct __wt_ikey {
    //key³¤¶È
     ...
     */
    //¼ÇÂ¼KÔÚÄÚ´æÖÐµÄÎ»ÖÃ£¬²Î¿¼__wt_row_ikey_alloc
    //²»Îª0Ôò¶ÔÓ¦´ÅÅÌpageÊý¾Ý¼ÓÔØµ½ÄÚ´æÖÐpage->dskµÄÎ»ÖÃ
    //±íÊ¾¸Ãref key¶ÔÓ¦µÄrefÏÂÃæµÄpageÒÑ¾­Ð´Èëµ½´ÅÅÌ»òÕß´Ó´ÅÅÌ¼ÓÔØµÄ
    uint32_t cell_offset;
...
//__wt_insert.updÎª¸ÃÀàÐÍ,¼ÇÂ¼µÄÊÇVµÄ±ä»¯¹ý³Ì
struct __wt_update {
    /*
     * Transaction IDs are set when updates are created (before they become visible) and only change
     * when marked with WT_TXN_ABORTED. Transaction ID readers expect to copy a transaction ID into
    ...
    //Ö»ÓÐÉèÖÃÁËcommit_timestamp²¢ÇÒÃ»ÓÐÆôÓÃWAL¹¦ÄÜ£¬durable_tsºÍstart_ts¹¦ÄÜ²Å»áÓÐÐ§£¬²Î¿¼__wt_txn_op_set_timestamp
    //__wt_txn_commit->__wt_txn_op_set_timestamp¼ÇÂ¼¸Ã²Ù×÷ÔÚÊÂÎñÖÐÌá½»µÄÊ±¼äµãdurable_timestamp¼ÇÂ¼µ½durable_tsÖÐ
    wt_timestamp_t durable_ts; /* timestamps */
    //__wt_txn_commit->__wt_txn_op_set_timestamp¼ÇÂ¼¸Ã²Ù×÷ÔÚÊÂÎñÖÐÌá½»µÄÊ±¼äµãcommit_timestamp¼ÇÂ¼µ½start_tsÖÐ
...
//__wt_hs_insert_updatesÖÐÉèÖÃ
#define WT_UPDATE_MODIFY 1    /* partial-update modify value */
//__wt_btcur_reserveÖÐÉèÖÃ£¬¿ÉÒÔÏÈ²»¹Ø×¢
 ...
 */ //__wt_cursor_btree.modify_updateÎª¸ÃÀàÐÍ
...
//mod_row_insertÎª¸ÃÀàÐÍ£¬Ò»¸öinsertKV¶ÔÓ¦µÄ__wt_insert¶¼ÔÚmod_row_insert¶ÔÓ¦ÌøÔ¾±íÖÐ
struct __wt_insert {
    ...
    union {
        ...
        } key; //key¼ÇÂ¼µ½ÕâÀï£¬valueÔÚupdÖÐ
    } u;
