...
//__evict_reconcile  __wt_evict_fileÖĞÖÃÎ»
#define WT_REC_EVICT 0x020u
//__evict_reconcileÖĞÈç¹ûÊÇleaf pageÉèÖÃ¸Ã±êÊ¶, __wt_sync_fileÒ²»áÉèÖÃ
#define WT_REC_HS 0x040u
#define WT_REC_IN_MEMORY 0x080u
//__evict_update_work ÀıÈçÈç¹ûÒÑÊ¹ÓÃÄÚ´æÕ¼±È×ÜÄÚ´æ²»³¬¹ı(target 80% + trigger 95%)ÅäÖÃµÄÒ»°ë£¬ÔòÉèÖÃ±êÊ¶WT_CACHE_EVICT_SCRUB£¬
//  ËµÃ÷reconcileµÄÊÊºÏ¿ÉÒÔÄÚ´æ¿½±´Ò»·İpageÊı¾İ´æÈëimage

//Ò»°ãreconcile¶¼»áÓµÓĞ¸Ã±êÊ¶£¬¼û__evict_reconcile
...
//WT_PAGE_HEADER(__wt_page_header)ÔÚ__rec_split_write_headerÖĞÍê³É¸³Öµ£¬¶ÔÓ¦ÄÚ´æÎ»ÖÃ¼ûWT_BLOCK_HEADER_REF
//WT_BLOCK_HEADER(__wt_block_header)¸³ÖµÔÚĞ´´ÅÅÌÍê³Éºó£¬ÔÚ__block_write_off¸³Öµ,¶ÔÓ¦ÄÚ´æÆ«ÒÆ¼ûWT_PAGE_HEADER_BYTE_SIZE
//Èç¹ûÅäÖÃÁËÑ¹Ëõ£¬Ôò__wt_page_header¼ÇÂ¼µÄÊÇÑ¹ËõÇ°µÄÊı¾İĞÅÏ¢,__wt_block_header¼ÇÂ¼µÄÊÇÑ¹ËõºóµÄÊı¾İĞÅÏ¢

//wtÎÄ¼şÍ·²¿4K magic  checksum¼ì²éÔÚ__desc_read

 
//Ò»¸öpageÊı¾İÔÚ´ÅÅÌÖĞÁ¬Ğø¿Õ¼äÄÚÈİ: WT_BLOCK_HEADER + __wt_page_header +  WT_CELL
//·ÖÅä¿Õ¼äºÍ¸³Öµ¿ÉÒÔ²Î¿¼__wt_rec_cell_build_ovfl  __rec_split_write_header
//page->dskÎª¸ÃÀàĞÍ
struct __wt_page_header {
    ...
    uint64_t recno; /* 00-07: column-store starting recno */ //ĞĞ´æ´¢¸ÃÄÚÈİÎªWT_RECNO_OOB
    ...
    //Ò²¾ÍÊÇ¶ÔÓ¦pageÀàĞÍ£¬ÀıÈçWT_PAGE_ROW_LEAF WT_PAGE_BLOCK_MANAGER(±íÊ¾checkpointµÄext)
 ...
 */ 
//Ò²¾ÍÊÇÉÏÃæµÄ__wt_page_headerÍ·²¿½á¹¹³¤¶È  
...
//dsk¿ªÊ¼Ìø¹ıblock header(WT_BLOCK_HEADER_SIZE) + page header(WT_PAGE_HEADER_SIZE) ,Ò²¾ÍÊÇÕâ¸öpage¶ÔÓ¦µÄÊµ¼ÊÊı¾İÆğÊ¼µØÖ·
...
//²Î¿¼__wt_multi_to_ref, ´æµÄÊÇ¸Ãref pageĞ´Èë´ÅÅÌµÄextÔªÊı¾İĞÅÏ¢(objectid offset size  checksum)
//__wt_multi.addr, __wt_ref.addrÎª¸ÃÀàĞÍ£¬²Î¿¼__wt_multi_to_ref
struct __wt_addr {
    //¸³Öµ¼û__rec_split_write,ÕæÊµÀ´Ô´Êµ¼ÊÉÏÔÚ__rec_row_leaf_insert  __wt_rec_row_leaf->WT_TIME_AGGREGATE_UPDATEÖĞÍ³¼Æ¸³Öµ
    //×îÖÕ»á¸³Öµ¸ø¸¸pageµÄindex[]Êı×éÏÂÃæref->addr.ta£¬²¢Í¨¹ıinternal pageÔÚ__wt_rec_cell_build_addrº¯ÊıÖĞ³Ö¾Ã»¯µ½´ÅÅÌ
    WT_TIME_AGGREGATE ta;

    //±£´æchunk->imageĞ´Èë´ÅÅÌÊ±ºòµÄÔªÊı¾İĞÅÏ¢(objectid offset size  checksum), __rec_split_writeÖĞ¸³Öµ
    uint8_t *addr; /* Block-manager's cookie */
    //__wt_multi_to_ref
    uint8_t size;  /* Block-manager's cookie length */

#define WT_ADDR_INT 1     /* Internal page */
#define WT_ADDR_LEAF 2    /* Leaf page */ //Òç³öµÄKEY£¬Ò²¾ÍÊÇ´ókey,
#define WT_ADDR_LEAF_NO 3 /* Leaf page, no overflow */
    //__wt_multi_to_ref
    uint8_t type; //ºÍWT_RECONCILE.ovfl_items¶ÔÓ¦
...
struct __wt_save_upd {
    ...
    //Èç¹ûupd_select->updÁ´±íµÚÒ»¸öudpÊÇÉ¾³ı²Ù×÷£¬Ö¸ÏòÕâ¸öÉ¾³ıudp
    WT_UPDATE *onpage_upd;
    //Èç¹ûupd_select->updÁ´±íµÚÒ»¸öudpÊÇÉ¾³ı²Ù×÷£¬Ö¸ÏòÕâ¸öÉ¾³ıudp
    WT_UPDATE *onpage_tombstone;
    //¸³Öµ²Î¿¼__rec_update_save
    //Èç¹ûsupd_restoreÎªtrueËµÃ÷´æÔÚÈ«¾Öid²»¿É¼ûµÄudp£¬Èç¹ûsupd_restoreÎªfalseËµÃ÷ins->udp
    //Á´±íÉÏÃæ¶¼ÊÇÈ«¾Öid¿É¼ûµÄudpÁ´±í£¬µ«ÊÇ´æÔÚtimestamp²»ÊÇÈ«¾Ö¿É¼ûµÄudp
 ...
 */ //Ò»¸öpage²ğ·ÖÎª¶à¸öpageÊ±ºòÓÃµ½£¬¿ÉÒÔ²Î¿¼__rec_split_dump_keysµÄ±éÀú,__rec_split_writeÕâÀï´´½¨¿Õ¼äºÍ¸³Öµ
//¹ÜÀí´ÅÅÌpage imageµÄÄÚ´æÔªÊı¾İĞÅÏ¢£¬Ò»¸ö__wt_multi¶ÔÓ¦Ò»¸öpag ´ÅÅÌimage
struct __wt_multi {
    /*
     * Block's key: either a column-store record number or a row-store variable length byte string.
     */
    //page²ğ·Öºó¶ÔÓ¦µÄref key£¬²Î¿¼__rec_split_dump_keysµÄ´òÓ¡£¬¸³ÖµµÄµØ·½ÔÚ__rec_split_write
    union {
        uint64_t recno;
        WT_IKEY *ikey;
    } key;
    ...
    //´ÓÉÏÃæµÄ±¸×¢¿ÉÒÔ¿´³ö£¬reconcileÒ»¸ömulti¶ÔÓ¦Êı¾İĞ´Èë´ÅÅÌºó£¬»áÔÚ¿½±´Ò»·İµ½disk_imageÖĞ£¬×îÖÕÔÚ__wt_page_inmemÖĞ¸³Öµ¸øpage->dsk
    ...
    //__evict_update_work ÀıÈçÈç¹ûÒÑÊ¹ÓÃÄÚ´æÕ¼±È×ÜÄÚ´æ²»³¬¹ı(target 80% + trigger 95%)ÅäÖÃµÄÒ»°ë£¬ÔòÉèÖÃ±êÊ¶WT_CACHE_EVICT_SCRUB£¬
    //  ËµÃ÷reconcileµÄÊ±ºò¿ÉÒÔÄÚ´æ¿½±´Ò»·İpageÊı¾İ´æÈëimage, Ò»°ã´ú±íÄÚ´æ±È½Ï³ä×ã£¬Òò´Ë¿ÉÒÔ¿½±´Ò»·İµ½image£¬×îÖÕ±»¸³Öµ¸ø
    //  ÄÚ´æÖĞµÄsplitµÄpageµÄpage->dsk,²Î¿¼__wt_multi_to_ref->__split_multi_inmem->__wt_page_inmem

    //Èç¹ûÄÚ´æºÜ½ôÕÅ£¬ÔòreconcileµÄÊ±ºò»á³Ö¾Ã»¯µ½´ÅÅÌ£¬ÕâÊ±ºòdisk_imageÎªNULL£¬split²ğ·ÖºóµÄref->pageÎªNULL£¬ÕâÊ±ºòpageÉú³ÉÒÀ¿¿ÓÃ»§Ïß³Ì
    //  ÔÚ__page_readÖĞ¶ÁÈ¡´ÅÅÌÊı¾İµ½pageÖĞÍê³Éref->pageÄÚ´æ¿Õ¼äÉú³É
    void *disk_image;
     ...
     * image.
     */
    //¸³Öµ²Î¿¼__rec_supd_move  __rec_update_save
    //¼ÇÂ¼Õâ¸öreconcileµÄpageÉÏÃæ´æÔÚµÄ²»¿É¼ûµÄudpÁ´±íĞÅÏ¢£¬ÕâÀïÊÇÒ»¸öÊı×é£¬×îÖÕ»áÍ¨¹ı__rec_hs_wrapup°ÑÕâĞ©²»¿É¼ûµÄupd¼ÇÂ¼µ½wiredtigerHs.wtÎÄ¼şÖĞ
    WT_SAVE_UPD *supd;
    uint32_t supd_entries;
    //ËµÃ÷ÓĞ²»ÊÇÈ«¾Öid¿É¼ûµÄudp,¸³Öµ²Î¿¼__rec_update_save
...
//ÀıÈçleaf pageÖĞÌøÔ¾±íÖĞ½ÚµãÄÚÈİ¶¼ÔÚ¸ÃmodifyÖĞ, modÏà¹Ø¿Õ¼äÊÍ·Å¼û__free_page_modify
struct __wt_page_modify {
    /* The first unwritten transaction ID (approximate). */
    uint64_t first_dirty_txn;

    /* The transaction state last time eviction was attempted. */
    //__reconcile_save_evict_stateÖĞ¸³Öµ
...
#define mod_disk_image u1.r.disk_image
        ...
        //¹ÜÀí´ÅÅÌpage imageµÄÄÚ´æÔªÊı¾İĞÅÏ¢£¬Ò»¸ö__wt_multi¶ÔÓ¦Ò»¸öpag ´ÅÅÌimage
        struct {//×¢Òâ__wt_reconcile.multiºÍ__wt_page_modify.mod_multi_entries.multiµÄÇø±ğÁªÏµ
            //__rec_write_wrapupÖĞ¸³Öµ
...
#define mod_col_split_recno u2.column_leaf.split_recno
        struct {
            ...
            //WT_PAGE_ALLOC_AND_SWAP __wt_leaf_page_can_split·ÖÅä¿Õ¼ä
            WT_INSERT_HEAD **insert;

            /* Updated items for row-stores. */
            //pg_rowÖ¸Ïò´ÅÅÌKVÏà¹ØÊı¾İ(Êµ¼ÊÉÏÕâÀïÖ¸ÏòµÄÊÇÄÚ´æ£¬´Ó´ÅÅÌ¼ÓÔØµÄ»òÕß´ÓÄÚ´æĞ´Èë´ÅÅÌµÄ£¬ÄÚ´æºÍ´ÅÅÌ¶¼ÓĞÒ»·İ)£¬mod_row_insertÖ¸ÏòÄÚ´æÏà¹ØKVÊı¾İ£¬mod_row_update¼ÇÂ¼ÄÚ´æÖĞÍ¬Ò»¸öKµÄ±ä¸ü¹ı³Ì
            WT_UPDATE **update;
        } row_leaf;
...
//WT_ROW_INSERT_SLOT»ñÈ¡¶ÔÓ¦µÄÌøÔ¾±í£¬Êµ¼ÊÉÏinsertÊÇ¸öÊı×é£¬Êı×éÃ¿¸ö³ÉÔ±¶ÔÓ¦Ò»¸öÌøÔ¾±í£¬²Î¿¼__wt_leaf_page_can_split
...
#define WT_PAGE_DIRTY 2
    //±êÊ¶¸Ãpage×´Ì¬£¬ÊÇ·ñÊÇÔàpage£¬²Î¿¼__wt_page_only_modify_set
    uint32_t page_state;

#define WT_PM_REC_EMPTY 1      /* Reconciliation: no replacement */
#define WT_PM_REC_MULTIBLOCK 2 /* Reconciliation: multiple blocks */
//ÀıÈçÒ»¸öpageÓĞĞŞ¸Ä£¬µ«ÊÇĞŞÁËÒ»µãµã£¬ÕâÊ±ºòÒ»¸öpageÍêÈ«¹»ÁË£¬Ò»°ãupdateµÄÊÊºÏÕâÖÖºÜ³£¼û
#define WT_PM_REC_REPLACE 3    /* Reconciliation: single block */
    //¸´ÖÆ²Î¿¼__rec_write_wrapup
...
//__wt_page_alloc·ÖÅä¿Õ¼ä£¬Í¨¹ıWT_INTL_INDEX_GET(session, page, pindex);»ñÈ¡page¶ÔÓ¦µÄ__wt_page_index
//¿ÉÒÔ²Î¿¼__split_parent
struct __wt_page_index {/* Sanity check for a reasonable number of on-page keys. */
#define WT_INTERNAL_SPLIT_MIN_KEYS 100
    //ĞÂÔö²ã·ÖÁË¶àÉÙ×é£¬ÀıÈç10002¸ö×Ópage»áÄ¬ÈÏ·Ö10002/100×é
    uint32_t entries;
    uint32_t deleted_entries;
    //Ö¸ÏòÕæÊµµÄindexÊı×éµØÖ·
    WT_REF **index;
};
 ...
 */
//±éÀú»ñÈ¡internal pageËù°üº¬µÄËùÓĞ×Óref
...
//Ö¸Ïò¸ÃpageµÄref£¬²Î¿¼Í¼ĞÎ»¯https://github.com/wiredtiger/wiredtiger/wiki/In-Memory-Tree-Layout
#define pg_intl_parent_ref u.intl.parent_ref
#undef pg_intl_split_gen
//¼ÇÂ¼¸Ãinternal page·ÖÁÑµÄ´ÎÊı  __split_parentÖĞ¸³Öµ
#define pg_intl_split_gen u.intl.split_gen

/*
 * WT_PAGE --
 *	The WT_PAGE structure describes the in-memory page information.
 */ //__wt_page_alloc·ÖÆ¬¿Õ¼äºÍ¸³Öµ __wt_page_out¶Ôpage½øĞĞ¿Õ¼äÊÍ·Å
struct __wt_page {
    ...
    union {
         ...
         */
        //ÅäºÏÍ¼ĞÎ»¯ÔÄ¶Áhttps://github.com/wiredtiger/wiredtiger/wiki/In-Memory-Tree-Layout
        struct {
            //Ö¸Ïò¸¸½ÚµãµÄpage
 ...
 * destroyed.
 */
//²Î¿¼BTREEÊıÍ¼ĞÎ»¯ https://github.com/wiredtiger/wiredtiger/wiki/In-Memory-Tree-Layout
//ÎªºÎÊÇsafe°²È«µÄ£¬¿ÉÒÔ²Î¿¼WT_ENTER_PAGE_INDEX  WT_LEAVE_PAGE_INDEX  WT_WITH_PAGE_INDEX
//Ö®Àà¿ÉÒÔÑéÖ¤Ò»ÏÂ£¬¼ÙÉèÄ³¸öÓÃ»§Ïß³Ì²Ù×÷µÄpage³¬¹ıÏŞÖÆ£¬ÔÚevictÖĞ»áspliteÖØĞÂÉú³ÉĞŞ¸Äindex£¬ÕâÊ±ºòÓÃ»§Ïß³ÌµÄevictÂß¼­ÖĞÎÒÃÇsleep£¬
//evict serverÏß³Ì»áÔõÃ´Ñù?
...
//pageµÄ__indexÊı×é¸³Öµ£¬¼û__wt_page_alloc
...
//³¡¾°1:
// ¸ÃpageÃ»ÓĞÊı¾İÔÚ´ÅÅÌÉÏÃæ
//   cbt->slotÔòÖ±½ÓÓÃWT_ROW_INSERT_SLOT[0]Õâ¸öÌø±í£¬

//³¡¾°2:
// Èç¹û´ÅÅÌÓĞÊı¾İÔòWT_ROW_INSERT_SMALLEST(mod_row_insert[(page)->entries])±íÊ¾Ğ´ÈëÕâ¸öpage²¢ÇÒKĞ¡ÓÚ¸ÃpageÔÚ
//   ´ÅÅÌÉÏÃæ×îĞ¡µÄKµÄËùÓĞÊı¾İÍ¨¹ıÕâ¸öÌø±í´æ´¢ÆğÀ´

//³¡¾°3:
// ¸ÃpageÓĞÊı¾İÔÚ´ÅÅÌÉÏÃæ£¬ÀıÈç¸ÃpageÔÚ´ÅÅÌÉÏÃæÓĞÁ½ÌìÊı¾İke1,key2...keyn£¬ĞÂ²åÈëkeyx>page×î´óµÄkeyn£¬ÔòÄÚ´æ»áÎ¬»¤Ò»¸ö
//   cbt->slotÎª´ÅÅÌKV×ÜÊı-1£¬ÕâÑù´óÓÚ¸ÃpageµÄËùÓĞKV¶¼»áÌí¼Óµ½WT_ROW_INSERT_SLOT[page->entries - 1]Õâ¸öÌø±íÉÏÃæ

//³¡¾°4:
// ¸ÃpageÓĞÊı¾İÔÚ´ÅÅÌÉÏÃæ£¬ÀıÈç¸ÃpageÔÚ´ÅÅÌÉÏÃæÓĞÁ½ÌìÊı¾İke1,key2,key3...keyn£¬ĞÂ²åÈëkeyx´óÓÚkey2Ğ¡ÓÚkey3, key2<keyx>key3£¬ÔòÄÚ´æ»áÎ¬»¤Ò»¸ö
//   cbt->slotÎª´ÅÅÌkey2ÔÚ´ÅÅÌÖĞµÄÎ»ÖÃ(Ò²¾ÍÊÇ1£¬´Ó0¿ªÊ¼Ëã)£¬ÕâÑù´óÓÚ¸ÃpageµÄËùÓĞKV¶¼»áÌí¼Óµ½WT_ROW_INSERT_SLOT[1]Õâ¸öÌø±í

//Ò»¸öpageÔÚ´ÅÅÌpage->pg_rowÓĞ¶àÉÙÊı¾İ(page->pg_row[]Êı×é´óĞ¡)£¬¾Í»áÎ¬»¤¶àÉÙ¸öÌø±í£¬ÒòÎªÒª±£Ö¤ĞÂĞ´ÈëÄÚ´æµÄÊı¾İºÍ´ÅÅÌµÄÊı¾İ±£³ÖË³Ğò
...
//__wt_block_extlist_write£¬±íÊ¾checkpointµÄext
...
//__evict_push_candidateÖÃÎ»£¬ÌôÑ¡³ö½øÈëevict¶ÓÁĞµÄpage±êÊ¶
...
#define WT_PAGE_UPDATE_IGNORE 0x100u      /* Ignore updates on page discard */
    ...
    //__wt_cache_page_inmem_decrÖĞ×ö¼õ·¨£¬__wt_cache_page_inmem_incrÖĞ×ö¼Ó·¨
    //¸Ãpage¶ÔÓ¦µÄÄÚ´æ£¬°üÀ¨Ğ´Èë´ÅÅÌµÄ£¬¿ÉÒÔ²Î¿¼__evict_force_check
    ...
    //×¢Òâ__wt_rec_row_leafºÍ__wt_rec_row_intµÄ²îÒì£¬Ò»¸öÊÇ³Ö¾Ã»¯leaf page£¬Ò»¸öÊÇ³Ö¾Ã»¯internal page
    //  __wt_rec_row_int: ³Ö¾Ã»¯internal pageÓµÓĞµÄ×Óref page key¼°ÆäËùÓĞ×Ópage³Ö¾Ã»¯µ½´ÅÅÌµÄext(objectid offset size  checksum)ÔªÊı¾İĞÅÏ¢, 
    //      µ±ÖØÆô(__wt_btree_tree_open->__wt_page_inmem)»òÕß¶ÁÈ¡interanal pageÊı¾İ(__page_read)µÄÊ±ºò»á°ÑÕâĞ©³Ö¾Ã»¯µÄref page keyºÍËùÓĞ³Ö¾Ã»¯µÄpageÔªÊı¾İ¼ÓÔØµ½page->dskÖĞ
    //  __wt_rec_row_leaf: ³Ö¾Ã»¯leaf pageÊı¾İµ½´ÅÅÌ£¬Í¬Ê±°Ñ´ÅÅÌextÔªÊı¾İ(objectid offset size  checksum)ĞÅÏ¢Ìí¼Óµ½ref.addr
    const WT_PAGE_HEADER *dsk; //¸³Öµ¼û__wt_page_inmem£¬Ö¸Ïò´ÅÅÌÊı¾İ

    /* If/when the page is modified, we need lots more information. */
    //__wt_page.modify ¸³Öµ¼û__wt_page_modify_init
    //ÀıÈçleaf pageÖĞÌøÔ¾±íÖĞ½ÚµãÄÚÈİ¶¼ÔÚ¸ÃmodifyÖĞ£¬modÏà¹Ø¿Õ¼äÊÍ·Å¼û__free_page_modify£¬WT_PAGE_MODIFY page->modifyÏà¹Ø¿Õ¼äÊÍ·Å£¬°üÀ¨mod_row_insert mod_row_update mod_multiµÈ
 ...
 */
//³õÊ¼Öµ, ÀıÈçÒ»¸öpage½øĞĞevictµÄÊ±ºò£¬´ÓÒ»¸öpage²ğ·ÖÎª¶à¸öpage,¶à³öÀ´µÄpageµÄ³õÊ¼ÖµÒ²¾ÍÊÇWT_READGEN_NOTSET
...
//Âú×ãÕâ¸öÌõ¼şÔÚ__evict_entry_priorityÖĞÖ±½ÓÆÀ·ÖÎªWT_READGEN_OLDEST
//ÆÀ·ÖÔÚÕâ¸ö·¶Î§µÄpageĞèÒªÁ¢Âíreconcile£¬
// ÆÀ·ÖÔ½µÍ£¬ËµÃ÷½üÆÚ¸ÃpageÔ½Ã»ÓĞ±»·ÃÎÊ£¬ÀıÈç2µã·ÃÎÊµÄpage1±È1µã·ÃÎÊµÄpage2µÄÆÀ·Ö»á¸ß£¬²Î¿¼__wt_cache_read_gen_bump __wt_cache_read_gen_new
/* Any page set to the oldest generation should be discarded. */
#define WT_READGEN_EVICT_SOON(readgen) \
    ((readgen) != WT_READGEN_NOTSET && (readgen) < WT_READGEN_START_VALUE)
#define WT_READGEN_START_VALUE 100
#define WT_READGEN_STEP 100
    //__wt_cache.read_gen´ú±íÈ«¾ÖµÄread_gen£¬page.read_gen´ú±íÖ¸¶¨±íµÄ
     //__wt_cache_read_gen_new: Èç¹ûread_genÃ»ÓĞÉèÖÃ£¬Ôòevict serverÏß³ÌÔÚ__evict_walk_treeÖĞÑ¡³öĞèÒªevictµÄpageºóÍ¨¹ı¸Ãº¯ÊıÉú³Épage read_gen
    //__wt_cache_read_gen_bump: evict worker»òÕßappÓÃ»§Ïß³ÌÔÚ__wt_cache_read_gen_bumpÖĞ´Ó¶ÓÁĞÏû·ÑÕâ¸öpage reconcileµÄÊ±ºò¸³Öµ

    //__wt_page_alloc  __wt_page_evict_soonÖĞ³õÊ¼ÖµWT_READGEN_NOTSET£¬__wt_cache_read_gen_bump __wt_cache_read_gen_newÖĞĞŞ¸Ä¸³Öµ
    //´ú±í½üÆÚÊÇ·ñÓÃ»§Ïß³ÌÓĞ·ÃÎÊ¸Ãpage£¬Èç¹ûÖµÔ½´ó£¬ËµÃ÷½üÆÚ¸Ãpage±»ÓÃ»§Ïß³Ì·ÃÎÊ»îÔ¾£¬²Î¿¼__wt_cache_read_gen_new
    uint64_t read_gen;//evict serverÏß³ÌÍ¨¹ı¸ÃÖµ¶ÔÌôÑ¡µÄpage½øĞĞÆÀ·Ö£¬¼û__evict_entry_priority

    uint64_t cache_create_gen; /* Page create timestamp */
    //¸³Öµ¼û__evict_walk_tree£¬Ò²¾ÍÊÇµÚ¼¸ÂÖ__evict_passµÄÊ±ºò¸Ãpage±»ºóÌ¨evict serverÏß³ÌÑ¡ÖĞÌÔÌ­µÄ
...
#define WT_PREPARE_INIT              \
    0 /* Must be 0, as structures    \
         ...
         with 0. */
//¸³Öµ¼û__page_inmem_prepare_update  __wt_txn_prepare
//__wt_txn_prepare prepareµÄÊÂÎñ»áÊÍ·Å¿ìÕÕ£¬ÕâÑù¾Í»áÓ°ÏìevictºÍ¿É¼ûĞÔ£¬Òò´Ëprepare½ÚµãÊÍ·Å¿ìÕÕ£¬ÕâÑù¿ÉÒÔ¼õÇáÄÚ´æÑ¹Á¦
// µ«ÊÇ¿É¼ûĞÔ»á²»Ò»ÖÂ£¬Òò´Ë»áÔö¼Ó¶ÔWT_PREPARE_INPROGRESSµÄWT_VISIBLE_PREPARE(¼û__wt_txn_upd_visible_type)£¬²¢Ôö¼Óudp
// WT_PREPARE_CONFLICT³åÍ»¼ì²é(¼û__wt_txn_read_upd_list_internal)
#define WT_PREPARE_INPROGRESS 1
//¸³Öµ¼û__txn_resolve_prepared_update
#define WT_PREPARE_LOCKED 2
//¸³Öµ¼û__txn_resolve_prepared_update  °ÑudpÁ´±íÉÏÃæÊôÓÚ±¾sessionÊÂÎñÕıÔÚ²Ù×÷µÄudp½ÚµãµÄ×´Ì¬ÖÃÎªWT_PREPARE_RESOLVED
 ...
 *	A single in-memory page and state information.
 ...
 BTREEÍ¼ĞÎ»¯²Î¿¼https://github.com/wiredtiger/wiredtiger/wiki/In-Memory-Tree-Layout
...
struct __wt_ref {
    ...
    //´ú±í¸ÃrefÔÚ¸¸page index[]Êı×éµÄÎ»ÖÃ
...
//±êÊ¶¸ÃpageÊı¾İÔÚdiskÖĞ,evict°ÑÄÚ´æpageĞ´Èë´ÅÅÌºóÔÚ__wt_multi_to_refÖĞÖÃÎª¸Ã×´Ì¬
#define WT_REF_DISK 0       /* Page is on disk */
//__btree_tree_open_empty´´½¨root pageµÄÊ±ºò³õÊ¼»¯Îª¸ÃÖµ
#define WT_REF_DELETED 1    /* Page is on disk, but deleted */
//evict workerÏß³ÌÔÚ__evict_get_refÖĞÉèÖÃpage×´Ì¬ÎªWT_REF_LOCKED 
//ÓÃ»§Ïß³ÌÔÚ__wt_page_release_evictÖĞÉèÖÃpage×´Ì¬ÎªWT_REF_LOCKED 
//ÔÚÕû¸ö__wt_evictÆÚ¼ä£¬ËùÓĞ·ÃÎÊ¸ÃpageµÄÓÃ»§Ïß³Ì¶¼»áÔÚ__wt_page_in_funcÖĞÒòÎª´¦ÓÚWT_REF_LOCKED¶øÒ»Ö±µÈ´ı£¬ÒòÎªsplit»áÆÆ»µÔ­ÓĞµÄref page½á¹¹£¬Ò»²ğ¶à
...
//__split_parent_discard_refÖÃÎª¸Ã×´Ì¬£¬ËµÃ÷¸Ãparent refÒòÎªsplitĞèÒª±»ÊÍ·Å
//__wt_page_release_evict½øĞĞsplitµÄÊ±ºò»á½øÈë¸Ã×´Ì¬£¬Èç¹û__wt_page_release_evict split³É¹¦£¬Ôò²ğ·ÖÇ°µÄref×´Ì¬»áÔÚ
//  __split_multi->__wt_multi_to_refÉú³ÉĞÂµÄref_new[]£¬Ô­À´µÄref»áÍ¨¹ı__split_safe_free->__wt_stash_add¼ÓÈëµÈ´ıÊÍ·Å¶ÓÁĞ²¢ÖÃÎªWT_REF_SPLIT
//  ÏÂÒ»ÂÖwhile»áÅĞ¶ÏWT_REF_SPLITÈ»ºó·µ»ØWT_REF_SPLIT£¬×îÖÕÔÚÍâ²ã__wt_txn_commit->__wt_txn_release->__wt_stash_discardÕæÕıÊÍ·Å
#define WT_REF_SPLIT 4      /* Parent page split (WT_REF dead) */
    //WT_REF_SET_STATE WT_REF_CAS_STATE        WT_REF_DISK WT_REF_MEM(´ÅÅÌ»¹ÊÇÄÚ´æ)¸³Öµ
     ...
     */
    //Í¨¹ıref->addr¿ÉÒÔÅĞ¶Ï³ı¸Ãref¶ÔÓ¦pageÊÇ·ñÂŞÅÌÁË __wt_ref_block_free
    //ÀıÈçevict reconcileÁ÷³ÌÖĞµÄ__wt_multi_to_ref£¬Ö¸Ïò¸Ãpage¶ÔÓ¦µÄ´ÅÅÌextÔªÊı¾İĞÅÏ¢WT_ADDR(objectid offset size  checksum)
    //Èç¹ûÊÇroot page£¬Æäaddr=NULL, Èç¹ûpageÔÚÄÚ´æÖĞ£¬ÀıÈçinternal page(²»°üÀ¨root page)»òÕßleaf pageÔÚÄÚ´æÖĞ£¬»¹Ã»ÓĞ³Ö¾Ã»¯

    //ÖØÆôÊ±ºò´Ó__inmem_row_intÖĞ´ÓrootÔªÊı¾İextÖĞ¼ÓÔØÃ¿¸öpageµÄ´ÅÅÌaddrĞÅÏ¢
//¿ÉÒÔ²Î¿¼ÔÚ__wt_sync_fileÖĞ»á±éÀúËùÓĞµÄbtree¹ı³Ì£¬¿´__wt_sync_fileÊÇÈçºÎ±éÀúbtreeµÄ£¬È»ºó×ßµ½ÕâÀï
/*
                        root page
                        /         \
                      /             \
...
ÉÏÃæÕâÒ»¿ÃÊ÷µÄ±éÀúË³Ğò: leaf1->leaf2->internal1->leaf3->leaf4->internal2->root
*/
//´ÓÉÏÃæµÄÍ¼¿ÉÒÔ¿´³ö£¬internal page(root+internal1+internal2)×Ü¹²Èı´Î×ßµ½ÕâÀï, internal1¼ÇÂ¼leaf1ºÍleaf2µÄpage addrÔªÊı¾İ[ref key, leaf page extÔªÊı¾İ]
//  internal2¼ÇÂ¼leaf3ºÍleaf4µÄpage addrÔªÊı¾İ[ref key, leaf page extÔªÊı¾İ],
//  root¼ÇÂ¼internal1ºÍinternal2µÄ addrÔªÊı¾İ[ref key, leaf page extÔªÊı¾İ],

    //×¢Òâ__wt_rec_row_leafºÍ__wt_rec_row_intµÄ²îÒì£¬Ò»¸öÊÇ³Ö¾Ã»¯leaf page£¬Ò»¸öÊÇ³Ö¾Ã»¯internal page
    //  __wt_rec_row_int: ³Ö¾Ã»¯internal pageÓµÓĞµÄ×Óref page key¼°ÆäËùÓĞ×Ópage³Ö¾Ã»¯µ½´ÅÅÌµÄext(objectid offset size  checksum)ÔªÊı¾İĞÅÏ¢, 
    //      µ±ÖØÆô(__wt_btree_tree_open->__wt_page_inmem)»òÕß¶ÁÈ¡interanal pageÊı¾İ(__page_read)µÄÊ±ºò»á°ÑÕâĞ©³Ö¾Ã»¯µÄref page keyºÍËùÓĞ³Ö¾Ã»¯µÄpageÔªÊı¾İ¼ÓÔØµ½page->dskÖĞ
    //  __wt_rec_row_leaf: ³Ö¾Ã»¯leaf pageÊı¾İµ½´ÅÅÌ£¬Í¬Ê±°Ñ´ÅÅÌextÔªÊı¾İ(objectid offset size  checksum)ĞÅÏ¢Ìí¼Óµ½ref.addr
    void *addr;//¶ÔÓ¦WT_ADDR£¬²Î¿¼__wt_multi_to_ref, ´æµÄÊÇ¸Ãref pageĞ´Èë´ÅÅÌµÄextÔªÊı¾İĞÅÏ¢(objectid offset size  checksum)
...
#undef ref_ikey
////WT_REF.ref_ikey ref¶ÔÓ¦µÄkey£¬__wt_row_ikey  __wt_ref_key_onpage_setÖĞ¸³Öµ
//¼ÇÂ¼µÄÊÇ¶ÔÓ¦pageµÄ×îĞ¡key
 ...
 */
//´ú±í´ÅÅÌÉÏÃæµÄÒ»ÌõKVÊı¾İ£¬²Î¿¼__debug_page_row_leaf
struct __wt_row { /* On-page key, on-page cell, or off-page WT_IKEY */
    //ÕæÕı¸³ÖµÔÚWT_ROW_KEY_SET
    void *volatile __key;
};
 ...
 */
//pg_rowÖ¸Ïò´ÅÅÌKVÏà¹ØÊı¾İ£¬mod_row_insertÖ¸ÏòÄÚ´æÏà¹ØKVÊı¾İ£¬mod_row_update¼ÇÂ¼ÄÚ´æÖĞÍ¬Ò»¸öKµÄ±ä¸ü¹ı³Ì
 ...
 */
//Ò²¾ÍÊÇ±êÊ¶rip¶ÔÓ¦KVÔÚpageÄÚ´æpg_rowÖĞµÄÓÎ±ê
 ...
 */ //__wt_row_ikey_alloc²Î¿¼£¬ref´æµÄÕæÊµpageÉÏ×îĞ¡Êı¾İkey
struct __wt_ikey {
    //key³¤¶È
     ...
     */
    //¼ÇÂ¼KÔÚÄÚ´æÖĞµÄÎ»ÖÃ£¬²Î¿¼__wt_row_ikey_alloc
    //²»Îª0Ôò¶ÔÓ¦´ÅÅÌpageÊı¾İ¼ÓÔØµ½ÄÚ´æÖĞpage->dskµÄÎ»ÖÃ
    //±íÊ¾¸Ãref key¶ÔÓ¦µÄrefÏÂÃæµÄpageÒÑ¾­Ğ´Èëµ½´ÅÅÌ»òÕß´Ó´ÅÅÌ¼ÓÔØµÄ
    uint32_t cell_offset;
...
//·ÖÅä__wt_upd_alloc¿Õ¼ä //KVÖĞµÄkey¶ÔÓ¦WT_INSERT£¬value¶ÔÓ¦WT_UPDATE(WT_INSERT.upd)
//__wt_insert.updÎª¸ÃÀàĞÍ,¼ÇÂ¼µÄÊÇVµÄ±ä»¯¹ı³Ì
struct __wt_update {
    /*
     * Transaction IDs are set when updates are created (before they become visible) and only change
     * when marked with WT_TXN_ABORTED. Transaction ID readers expect to copy a transaction ID into
    ...
    //ÊÂÎñÌá½»µÄÊ±ºò»áÔÚ__wt_txn_commit->__wt_txn_releaseÖĞÖÃsession¶ÔÓ¦ÊÂÎñid WT_SESSION_IMPL->txn->idÎªWT_TXN_NONE 
    //upd->txnidÎªĞŞ¸Ä¸ÃÖµ¶ÔÓ¦µÄÊÂÎñid(__wt_txn_modify)£¬¸ÃidÖµ²»»áÒòÎªÊÂÎñÌá½»ÖÃÎª0
     
    //¸Ãupd¶ÔÓ¦µÄÊÂÎñid£¬¸³Öµ¼û__wt_txn_modify,Í¬Ò»¸öÊÂÎñÖĞµÄĞ´²Ù×÷txnidÒ»Ñù
    volatile uint64_t txnid; /* transaction ID */


    //×¢ÒâÖ»ÓĞ¹Ø±ÕÁËoplog(log=(enabled=false))¹¦ÄÜµÄ±í²Å»áÓĞupd timestamp¹¦ÄÜ
    //Ö»ÓĞÉèÖÃÁËcommit_timestamp²¢ÇÒÃ»ÓĞÆôÓÃWAL¹¦ÄÜ£¬durable_tsºÍstart_ts¹¦ÄÜ²Å»áÓĞĞ§£¬²Î¿¼__wt_txn_op_set_timestamp
    //__wt_txn_op_set_timestamp¼ÇÂ¼¸Ã²Ù×÷ÔÚÊÂÎñÖĞÌá½»µÄÊ±¼äµãdurable_timestamp¼ÇÂ¼µ½durable_tsÖĞ,×îÖÕÔÚWT_TIME_WINDOW_SET_STARTÖĞÊ¹ÓÃ
    wt_timestamp_t durable_ts; /* timestamps */ //Èç¹ûÊÇprepare_transactionÔò¸³ÖµÔÚ__txn_resolve_prepared_update
    //__wt_txn_op_set_timestamp¼ÇÂ¼¸Ã²Ù×÷ÔÚÊÂÎñÖĞÌá½»µÄÊ±¼äµãcommit_timestamp¼ÇÂ¼µ½start_tsÖĞ,×îÖÕÔÚWT_TIME_WINDOW_SET_STOPÖĞÊ¹ÓÃ
    //×¢ÒâÖ»ÓĞ¹Ø±ÕÁËoplog(log=(enabled=false))¹¦ÄÜµÄ±í²Å»áÓĞupd timestamp¹¦ÄÜ
    wt_timestamp_t start_ts; //Èç¹ûÊÇprepare_transactionÔò¸³ÖµÔÚ__txn_resolve_prepared_update  __wt_txn_prepareºÍ__wt_txn_op_delete_apply_prepare_state
...
//__wt_hs_insert_updatesÖĞÉèÖÃ
#define WT_UPDATE_MODIFY 1    /* partial-update modify value */
//__wt_btcur_reserveÖĞÉèÖÃ£¬¿ÉÒÔÏÈ²»¹Ø×¢
#define WT_UPDATE_RESERVE 2   /* reserved */
//Ò»°ã¶¼ÊÇÕâ¸ö×´Ì¬,ÆÕÍ¨¸üĞÂ
#define WT_UPDATE_STANDARD 3  /* complete value */
//ËµÃ÷ÊÇÉ¾³ı²Ù×÷ __wt_upd_alloc_tombstone´´½¨delete udp
#define WT_UPDATE_TOMBSTONE 4 /* deleted */
    //´ú±íÊÇÊ²Ã´ÀàĞÍµÄ¸üĞÂ£¬ÊÇÆÕÍ¨¸üĞÂ»¹ÊÇÉ¾³ı²Ù×÷£¬»¹ÊÇ²¿·Ö¸üĞÂ
 ...
 */ //__wt_cursor_btree.modify_updateÎª¸ÃÀàĞÍ
struct __wt_update_value {
    WT_ITEM buf;
    //¸³Öµ¿ÉÒÔ²Î¿¼__wt_upd_value_assign
    WT_TIME_WINDOW tw;
    uint8_t type;
    bool skip_buf;
};
 ...
 */
//__wt_row_insert_alloc  WT_INSERTÍ·²¿+level¿Õ¼ä+ÕæÊµÊı¾İkey
//mod_row_insertÎª¸ÃÀàĞÍ£¬Ò»¸öinsertKV¶ÔÓ¦µÄ__wt_insert¶¼ÔÚmod_row_insert¶ÔÓ¦ÌøÔ¾±íÖĞ
struct __wt_insert {
    //ÊÇÒ»¸öÁ´±í½á¹¹£¬´æ´¢²åÈë½øÀ´µÄÕâ¸öKµÄ¶à¸öV°æ±¾
    WT_UPDATE *upd; /* value */ //valueÔÚÕâÀï  __wt_insert.updÎª¸ÃÀàĞÍ
    ...
    union {
        ...
        } key; //key¼ÇÂ¼µ½ÕâÀï£¬valueÔÚupdÖĞ
    } u;
...
#define WT_INSERT_RECNO(ins) (((WT_INSERT *)(ins))->u.recno)
    ...
    //Êı×é´óĞ¡ºÍËæ»ú²ãÊıÏà¹Ø£¬²Î¿¼__wt_row_insert_alloc
 ...
 * 	The head of a skiplist of WT_INSERT items.
 ...
 ÌøÔ¾±íÍ¼½â²Î¿¼https://www.jb51.net/article/199510.htm
 */ //WT_PAGE_ALLOC_AND_SWAPÖĞ»á·ÖÅä¿Õ¼ä
...
//ËµÃ÷Ã»ÓĞÊı¾İÔÚ´ÅÅÌÉÏÃæ£¬ÔòĞ´ÈëµÄÊı¾İÖ±½ÓÌí¼Óµ½mod_row_insert[0]Õâ¸öÌø±í£¬Èç¹û´ÅÅÌÓĞÊı¾İÔòWT_ROW_INSERT_SMALLEST±íÊ¾Ğ´ÈëÕâ¸öpage
// ²¢ÇÒKĞ¡ÓÚ¸ÃpageÔÚ´ÅÅÌÉÏÃæ×îĞ¡µÄKµÄËùÓĞÊı¾İÍ¨¹ıÕâ¸öÌø±í´æ´¢ÆğÀ´
 ...
 * free it.
 */
//ÓÃ»§±£»¤page index, WT_INTL_INDEX_GET_SAFE
