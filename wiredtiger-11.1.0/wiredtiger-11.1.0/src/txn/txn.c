...
__wt_verbose_dump_txn(WT_SESSION_IMPL *session, const char *func_name)
{
   ...
   // WT_DECL_ITEM(txn_one_buf);
    ...
    //Èç¹ûÃ»ÓÐÏÖÊµÉèÖÃdurable_timestamp£¬Ã¿´ÎÊÂÎñÌá½»µÄÊ±ºò¶¼»áÄ¬ÈÏºÍcommit_timestamp±£³ÖÒ»ÖÂ
    ...
    //mongodb»áÉèÖÃoldest_timestamp=stable_timestamp-minSnapshotHistoryWindowInSeconds
    ...
    //pinned_timestampÊÇÔÚ__wt_txn_update_pinned_timestampÖÐ¸ù¾Ýoldest_timestampºÍËùÓÐÊÂÎñµÄread_timestampÈ¡×îÐ¡ÖµËã³öÀ´µÄ£¬(mongodb txnOpen.setReadSnapshot(_readAtTimestamp);)
    //Êµ¼ÊÉÏÕæÊµÓÃ´¦ÈçÏÂ:
    //  1. Ö»»áÔÚ__wt_update_obsolete_checkÖÐµ±Ò»¸öKÉÏÃæµÄÀúÊ·°æ±¾³¬¹ý20¸öµÄÊ±ºò²Å»áÓ°Ïì¸ÃupdateµÄ¼ì²é
    //  2. __wt_txn_visible_all->__wt_txn_pinned_timestampÓÃpinned_timestampÀ´ÅÐ¶ÏÊÂÎñ¿É¼ûÐÔ£¬½ø¶øÓ°ÏìÀúÊ·°æ±¾Êý¾Ý´ÓÄÚ´æÊÍ·Å
