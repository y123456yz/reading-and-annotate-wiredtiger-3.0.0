...
__curversion_get_value(WT_CURSOR *cursor, ...)
{
    ...
    CURSOR_API_CALL(cursor, session, __curversion_get_value, NULL);
...
__curversion_next(WT_CURSOR *cursor)
{
    ...
    CURSOR_API_CALL(cursor, session, __curversion_next, CUR2BT(version_cursor->file_cursor));
...
__curversion_reset(WT_CURSOR *cursor)
{
    ...
    hs_cursor = version_cursor->hs_cursor;
    file_cursor = version_cursor->file_cursor;
    CURSOR_API_CALL(cursor, session, __curversion_reset, NULL);
...
__curversion_search(WT_CURSOR *cursor)
{
    ...
    CURSOR_API_CALL(cursor, session, __curversion_search, CUR2BT(cbt));
    txn = session->txn;
...
__curversion_close(WT_CURSOR *cursor)
{
    ...
    hs_cursor = version_cursor->hs_cursor;
    file_cursor = version_cursor->file_cursor;
    CURSOR_API_CALL(cursor, session, __curversion_close, NULL);
