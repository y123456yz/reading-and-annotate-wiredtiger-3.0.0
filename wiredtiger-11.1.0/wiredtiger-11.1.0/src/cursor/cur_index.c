...
__curindex_get_value(WT_CURSOR *cursor, ...)
{
    ...
    JOINABLE_CURSOR_API_CALL(cursor, session, __curindex_get_value, NULL);
...
__curindex_set_valuev(WT_CURSOR *cursor, va_list ap)
{
    ...
    JOINABLE_CURSOR_API_CALL(cursor, session, __curindex_set_valuev, NULL);
...
__curindex_compare(WT_CURSOR *a, WT_CURSOR *b, int *cmpp)
{
    ...
    JOINABLE_CURSOR_API_CALL(a, session, __curindex_compare, NULL);
...
__curindex_next(WT_CURSOR *cursor)
{
    ...
    JOINABLE_CURSOR_API_CALL(cursor, session, __curindex_next, NULL);
...
__curindex_prev(WT_CURSOR *cursor)
{
    ...
    cindex = (WT_CURSOR_INDEX *)cursor;
    JOINABLE_CURSOR_API_CALL(cursor, session, __curindex_prev, NULL);
    F_CLR(cursor, WT_CURSTD_KEY_SET | WT_CURSTD_VALUE_SET);
...
__curindex_search(WT_CURSOR *cursor)
{
    ...
    cindex = (WT_CURSOR_INDEX *)cursor;
    child = cindex->child;
    JOINABLE_CURSOR_API_CALL(cursor, session, __curindex_search, NULL);
...
__curindex_search_near(WT_CURSOR *cursor, int *exact)
{
    ...
    cindex = (WT_CURSOR_INDEX *)cursor;
    child = cindex->child;
    JOINABLE_CURSOR_API_CALL(cursor, session, __curindex_search_near, NULL);
...
__curindex_bound(WT_CURSOR *cursor, const char *config)
{
    ...
    JOINABLE_CURSOR_API_CALL_CONF(cursor, session, bound, __curindex_bound, config, cfg, NULL);
