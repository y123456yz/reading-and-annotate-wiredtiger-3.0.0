...
access_example2(void)
{
    ...
    /* Open a connection to the database, creating it if necessary. */
    error_check(wiredtiger_open(home, NULL, "create,statistics=(all),log=(enabled:true,recover=on,remove=true),"
    "verbose=[all:5,checkpoint:5,timestamp:5,transaction:5, metadata:0, recovery:5, recovery_progress:5, log:5]", &conn));
    ...
    */
    ...
    error_check(session->checkpoint(session, "name=midnight")); 
    cursor->set_key(cursor, "key2");  
    cursor->set_value(cursor, "value22222");
    error_check(cursor->update(cursor));
    
    cursor->set_key(cursor, "key1");  
    cursor->set_value(cursor, "value11111111111111111111");
    error_check(cursor->update(cursor));
    //error_check(session->checkpoint(session, "name=midnight2")); 
    ...
    */
    ...
    while ((ret = cursor->next(cursor)) == 0) {
        ...
        printf("Got cursor midnight record: %s : %s\n", key, value);
    }

    cursor->set_key(cursor, "key3");  
    cursor->set_value(cursor, "value33333333333333333333");
    error_check(cursor->update(cursor));
    error_check(session->checkpoint(session, "name=midnight2")); 
    
    cursor->set_key(cursor, "key4");  
    ...
    */
    ...
    while ((ret = cursor->next(cursor)) == 0) {
        ...
        printf("Got cursor midnight2 record: %s : %s\n", key, value);
    }
    __wt_sleep(1, 10000);
    exit(0);
