...
compare_backups(int i)
{
    ...
    if (i == 0) {
        ...
        (void)snprintf(buf, sizeof(buf), "../../wt -R -h %s.%d dump main > %s.%d", home_full, i, full_out, i));
        error_check(system(buf));
     ...
     */
    (void)snprintf(buf, sizeof(buf), "../../wt -R -h %s.%d dump main > %s.%d", home_incr, i, incr_out, i));
    error_check(system(buf));
    ...
    if (0) {
        (void)snprintf(buf, sizeof(buf), "rm -rf %s.%d %s.%d %s.%d %s.%d", home_full, i, home_incr, i, full_out, i, incr_out, i);
        error_check(system(buf));
    }
    return (ret);
}
...
setup_directories(void)
{
    ...
    for (i = 0; i < MAX_ITERATIONS; i++) {
         ...
         * original at the end.
         */
        (void)snprintf(buf, sizeof(buf), "rm -rf %s.%d && mkdir -p %s.%d/%s", home_incr, i, home_incr, i, logpath));
        error_check(system(buf));
        if (i == 0)
            continue;
        /*
         ...
         */
        (void)snprintf(buf, sizeof(buf), "rm -rf %s.%d && mkdir -p %s.%d/%s", home_full, i, home_full, i, logpath));
        error_check(system(buf));
    }
}
...
take_incr_backup(WT_SESSION *session, int i)
{
   ...
   // return; //yang add change
    ...
    while ((ret = backup_cur->next(backup_cur)) == 0) {
        ...
        if (strncmp(filename, WTLOG, WTLOGLEN) == 0)
            error_check(system("cp %s/%s/%s %s/%s/%s", home, logpath, filename, h, logpath, filename));
        else
            error_check("cp %s/%s %s/%s", home, filename, h, filename));
#if 0
        ...
        while ((ret = incr_cur->next(incr_cur)) == 0) {
            ...
            if (type == WT_BACKUP_RANGE) {
                ...
                if (strncmp(filename, WTLOG, WTLOGLEN) == 0)
                    ...
                    error_check(system("cp %s/%s %s/%s", home, filename, h, filename));
#if 1
