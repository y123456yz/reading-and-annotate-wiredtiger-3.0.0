...
access_example(int argc, char *argv[])
{
    ...
    if (insertConfig) {
        ...
        error_check(wiredtiger_open(home, NULL, "create,statistics=(all),verbose=[config_all_verbos:5, metadata:0, api:0]", &conn));
    ...
    if (loadDataConfig) {
        /* Open a connection to the database, creating it if necessary. */
        error_check(wiredtiger_open(home, NULL, "statistics=(all),verbose=[config_all_verbos:5, metadata:5, api:5]", &conn));
