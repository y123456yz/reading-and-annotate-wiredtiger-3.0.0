...
*/
// clear && ./ex_hello -i 1 运行会hang住   不打日志就正常，应该和我开发机有关
static void
access_example(int argc, char *argv[])
{
    ...
    if (insertConfig) {
        ...
        //error_check(wiredtiger_open(home, NULL, "create,eviction_target=96,eviction_trigger=99, verbose=[api:0, config_all_verbos:0, metadata:0, api:0]", &conn));
        error_check(wiredtiger_open(home, NULL, "create,eviction_target=96,eviction_trigger=99", &conn));
        ...
        testutil_check(conn->reconfigure(conn, "eviction_target=11,eviction_trigger=22, verbose=[config_all_verbos:0, metadata:0, api:0]"));
